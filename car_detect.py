...
def process_frame(frame):
    ...
    for scale in np.linspace(scale_step[0], scale_step[1], steps):
        out_img, boxes = find_cars(image, ystart, ystop, scale, clf, X_scaler, orient, pix_per_cell, cell_per_block,
...
X_train, X_test, y_train, y_test = train_test_split(X_all, y, test_size=0.2, random_state=rand_state)
...
joblib.dump(clf, 'model/svm_blank.pkl')
joblib.dump(X_scaler , 'model/X_scaler.pkl')

car_1 = mpimg.imread(cars[2])
car_2 = mpimg.imread(cars[1])
noncar_1 = mpimg.imread(noncars[3])
noncar_2 = mpimg.imread(noncars[1])
fig = plt.figure()
plt.subplot(121)
plt.imshow(car_1)
plt.title('Car 1')
...
car = mpimg.imread(cars[2])
img = convert_color(car, conv="RGB2YCrCb")
features, hog_img = get_hog_features(img[:,:,0], orient, pix_per_cell, cell_per_block, vis=True, feature_vec=False)
...
if __test_video__:
    white_output = './output_video/test_out_video.mp4'
